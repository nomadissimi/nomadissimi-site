    ...
    <main className="min-h-screen bg-[#FBF8F2] px-4 py-10 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-3xl space-y-6">
        ...
        <div className="rounded-[28px] border border-black/10 bg-white/70 p-8 shadow-[0_24px_80px_rgba(0,0,0,0.08)]">
          ...
          <div className="mt-6 rounded-[24px] border border-black/10 bg-[#FBF8F2] p-6">
            <h2 className="serif text-2xl text-black">Manage devices</h2>

            <p className="mt-3 sans text-[15px] leading-[1.8] text-black/60">
              For content protection, your portal access is limited to 2 active
              devices. When a newer device is added, the oldest active session
              may be signed out automatically.
            </p>

            <div className="mt-5 rounded-2xl border border-black/10 bg-white px-4 py-3">
              <p className="sans text-sm text-black/70">
                Active devices:{" "}
              ...
              {sessions.map((session, index) => (
                ...
                >
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      <p className="sans text-[15px] font-medium text-black/80">
                        {formatUserAgent(session.userAgent)}
                      </p>
                      <p className="mt-1 sans text-[13px] text-black/50">
                        Last seen: {formatDateTime(session.lastSeenAt)}
                      </p>
                      <p className="mt-1 sans text-[13px] text-black/40">
                        Session started: {formatDateTime(session.createdAt)}
                      </p>
                    </div>

                    <div className="flex items-center gap-2">
                      {session.isCurrent ? (
                        <span className="inline-flex items-center rounded-full border border-[#4B5D44]/20 bg-[#EEF5EA] px-3 py-1 sans text-[11px] uppercase tracking-[0.14em] text-[#4B5D44]">
                          Current device
                        </span>
                      ) : null}

                      <span className="inline-flex items-center rounded-full border border-black/10 bg-[#FBF8F2] px-3 py-1 sans text-[11px] uppercase tracking-[0.14em] text-black/45">
                        Device {index + 1}
                      </span>
                    </div>
                  </div>
            ...
            <p className="mt-4 sans text-[13px] leading-[1.7] text-black/40">
              Session controls are enforced automatically to protect portal
              content.
            </p>
          </div>
